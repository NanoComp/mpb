...
double normal_err_to_object(geometric_object o, double r, vector3 dir,
			    int numsq, real *x, real *y, real *z, real *w)
{
     int i;
     double d;
     ...
     for (i = 0; i < numsq; ++i) {
	  ...
	  if (point_in_fixed_objectp(vector3_plus(v, c), o))
	       nsum = vector3_plus(nsum, vector3_scale(w[i], v));
     }
		       ...
		       int numsq, real *x, real *y, real *z, real *w)
{
     ...
     printf("mean error angle for %d-pt formula = "
	    ...
	    numsq, err_mean, err_std, err_max);
}
...
int main(void)
{
     ...
     for (nsq = 0; nsq < sizeof(num_sq_pts) / sizeof(int); ++nsq) {
	  ...
	  for (i = 0; i < NUM_OBJECTS; ++i) {
	       ...
	       normals_to_object(o, 0.01, NUM_PLANES/100, num_sq_pts[nsq],x,y,z,w);
