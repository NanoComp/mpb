...
extern void Aop(evectmatrix Xin, evectmatrix Xout, void *data, int is_current_eigenvector,
                evectmatrix Work);
extern void Bop(evectmatrix Xin, evectmatrix Xout, void *data, int is_current_eigenvector,
                evectmatrix Work);
extern void Ainvop(evectmatrix Xin, evectmatrix Xout, void *data, evectmatrix Y, real *eigenvals,
                   sqmatrix YtY);
extern void Cop(evectmatrix Xin, evectmatrix Xout, void *data, evectmatrix Y, real *eigenvals,
                sqmatrix YtY);
...
real norm_diff(scalar *a, scalar *b, int n) {
  ...
  for (i = 0; i < n; ++i) {
    scalar d;
    ASSIGN_SCALAR(d, SCALAR_RE(b[i]) - SCALAR_RE(a[i]), SCALAR_IM(b[i]) - SCALAR_IM(a[i]));
...
void rand_posdef(sqmatrix A, sqmatrix X) {
  ...
  for (i = 0; i < n * n; ++i)
    ASSIGN_SCALAR(X.data[i], rand() * 1.0 / RAND_MAX, rand() * 1.0 / RAND_MAX);
  ...
  for (i = 0; i < n; ++i)
    ASSIGN_SCALAR(A.data[i * n + i], n * SCALAR_RE(A.data[i * n + i]),
...
int main(int argc, char **argv) {
  ...
  for (trial = 0; trial < 2; ++trial) {
    ...
    eigensolver(Y, eigvals, Aop, NULL, bop, NULL, Cop, NULL, NULL, NULL, W, nWork, 1e-10,
                &num_iters, EIGS_DEFAULT_FLAGS);
    printf("\nSolved for eigenvectors after %d iterations.\n", num_iters);
    printf("\nEigenvalues = ");
    for (sum = 0.0, i = 0; i < p; ++i) {
      sum += eigvals[i];
      printf("  %f", eigvals[i]);
      CHECK(fabs(eigvals[i] - eigvals_dense[i]) < 1e-5 * eigvals_dense[i], "incorrect eigenvalue");
    }
    printf("\nEigenvalue sum = %f\n", sum);
    ...
    evectmatrix_copy(Y, Ystart);
    eigensolver(Y, eigvals, Aop, NULL, bop, NULL, Ainvop, NULL, NULL, NULL, W, nWork, 1e-10,
                &num_iters, EIGS_DEFAULT_FLAGS);
    ...
    evectmatrix_copy(Y, Ystart);
    eigensolver(Y, eigvals, Aop, NULL, bop, NULL, Cop, NULL, NULL, NULL, W, nWork - 1, 1e-10,
                &num_iters, EIGS_DEFAULT_FLAGS);
    printf("Solved for eigenvectors after %d iterations.\n", num_iters);
    printf("\nEigenvalues = ");
    for (sum = 0.0, i = 0; i < p; ++i) {
    ...
    evectmatrix_copy(Y, Ystart);
    eigensolver(Y, eigvals, Aop, NULL, bop, NULL, NULL, NULL, NULL, NULL, W, nWork, 1e-10,
                &num_iters, EIGS_DEFAULT_FLAGS);
    printf("Solved for eigenvectors after %d iterations.\n", num_iters);
    printf("\nEigenvalues = ");
    for (sum = 0.0, i = 0; i < p; ++i) {
    ...
    evectmatrix_copy(Y, Ystart);
    eigensolver(Y, eigvals, Aop, NULL, bop, NULL, NULL, NULL, NULL, NULL, W, nWork - 1, 1e-10,
                &num_iters, EIGS_DEFAULT_FLAGS);
    printf("Solved for eigenvectors after %d iterations.\n", num_iters);
    printf("\nEigenvalues = ");
    for (sum = 0.0, i = 0; i < p; ++i) {
...
void Aop(evectmatrix Xin, evectmatrix Xout, void *data, int is_current_eigenvector,
         evectmatrix Work) {
  CHECK(A.p == Xin.n && A.p == Xout.n && Xin.p == Xout.p, "matrices not conformant");

  blasglue_gemm('N', 'N', Xout.n, Xout.p, Xin.n, 1.0, A.data, A.p, Xin.data, Xin.p, 0.0, Xout.data,
                Xout.p);
}

void Bop(evectmatrix Xin, evectmatrix Xout, void *data, int is_current_eigenvector,
         evectmatrix Work) {
  CHECK(B.p == Xin.n && B.p == Xout.n && Xin.p == Xout.p, "matrices not conformant");

  blasglue_gemm('N', 'N', Xout.n, Xout.p, Xin.n, 1.0, B.data, B.p, Xin.data, Xin.p, 0.0, Xout.data,
                Xout.p);
}

void Ainvop(evectmatrix Xin, evectmatrix Xout, void *data, evectmatrix Y, real *eigenvals,
            sqmatrix YtY) {
  CHECK(Ainv.p == Xin.n && Ainv.p == Xout.n && Xin.p == Xout.p, "matrices not conformant");

  blasglue_gemm('N', 'N', Xout.n, Xout.p, Xin.n, 1.0, Ainv.data, Ainv.p, Xin.data, Xin.p, 0.0,
                Xout.data, Xout.p);
}

void Cop_old(evectmatrix Xin, evectmatrix Xout, void *data, evectmatrix Y, real *eigenvals,
             sqmatrix YtY) {
  int in, ip;
  ...
  for (in = 0; in < Xout.n; ++in) {
    ...
    for (ip = 0; ip < Xout.p; ++ip) {
      ...
      ASSIGN_SCALAR(Xout.data[in * Xout.p + ip], diag * SCALAR_RE(xin), diag * SCALAR_IM(xin));
    }
  }
}

void Cop(evectmatrix Xin, evectmatrix Xout, void *data, evectmatrix Y, real *eigenvals,
         sqmatrix YtY) {
  int in, ip;
  ...
  for (in = 0; in < Xout.n; ++in) {
    ...
    for (ip = 0; ip < Xout.p; ++ip) {
      scalar scale;
      if (eigenvals) { ASSIGN_SCALAR(scale, SCALAR_RE(diag) - 0 * eigenvals[ip], SCALAR_IM(diag)); }
      else
        scale = diag;
      ASSIGN_DIV(Xout.data[in * Xout.p + ip], Xout.data[in * Xout.p + ip], scale);
    }
  }
}
