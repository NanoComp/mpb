...
static void epsilon(symmetric_matrix *eps, symmetric_matrix *eps_inv, const real r[3],
                    void *edata_v) {
  epsilon_data *edata = (epsilon_data *)edata_v;
  real eps_val;

#if INVERSION_SYM
  if (fabs(r[0]) < 0.5 * edata->eps_high_x || fabs(r[0] - 1.0) < 0.5 * edata->eps_high_x)
    eps_val = edata->eps_high;
#else
  if ((r[0] < edata->eps_high_x && r[0] >= 0.0) || (r[0] >= 1.0 && r[0] - 1.0 < edata->eps_high_x))
    eps_val = edata->eps_high;
#endif
  else
...
real bragg_omega(real omega_guess, real k, real n1, real f1, real n2, real f2, real tolerance) {
  real omega_guess_low = omega_guess - 0.2, omega_guess_high = omega_guess + 0.2;
  real k_cur;
  real k_best = -1.0, omega_best = 0.0;
  real tol;

  if (omega_guess_low < 0.0) omega_guess_low = 0.0;

  for (tol = (omega_guess_high - omega_guess_low) / 10.0; tol > tolerance; tol *= 0.25) {
    for (omega_guess = omega_guess_low + tol; omega_guess < omega_guess_high; omega_guess += tol) {
...
void usage(void) {
  printf("Syntax: maxwell_test [options]\n"
         ...
         "   -v           Verbose output.\n",
         KX, NUM_BANDS, sqrt(EPS_HIGH), EPS_HIGH_X, NX, NY, NZ, ERROR_TOL, MESH_SIZE);
}
...
int main(int argc, char **argv) {
...
#ifdef HAVE_GETOPT
  {
    ...
    while ((c = getopt(argc, argv, "hs:k:b:n:f:x:y:z:emt:c:g:1pvE:")) != -1)
      switch (c) {
        case 'h':
          usage();
          exit(EXIT_SUCCESS);
        ...
        case 'k': kvector[0] = atof(optarg); break;
        case 'b':
        ...
        case 'z':
          ...
          break;
        case 'e': parity = EVEN_Z_PARITY; break;
        case 'm': parity = ODD_Z_PARITY; break;
        case 't':
        ...
        case 'E':
          ...
          break;
        case 'c': error_tol = fabs(atof(optarg)); break;
        case 'g':
          mesh_size = atoi(optarg);
          CHECK(mesh_size > 0, "mesh size must be positive");
          break;
        case '1': stop1 = 1; break;
        case 'p': which_preconditioner = 1; break;
  ...
  mdata = create_maxwell_data(nx, ny, nz, &local_N, &N_start, &alloc_N, num_bands, NUM_FFT_BANDS);
  ...
  if (verbose && ny == 1 && nz == 1) {
    ...
    for (i = 0; i < nx; ++i) {
      if (mdata->eps_inv[i].m00 == mdata->eps_inv[i].m11)
        printf("  eps(%g) = %g\n", i * 1.0 / nx, 1.0 / mdata->eps_inv[i].m00);
  ...
  Hstart = create_evectmatrix(nx * ny * nz, 2, num_bands, local_N, N_start, alloc_N);
  for (i = 0; i < NWORK; ++i)
    W[i] = create_evectmatrix(nx * ny * nz, 2, num_bands, local_N, N_start, alloc_N);
  ...
  for (iters = 0; iters < PROF_ITERS; ++iters) {
    ...
    eigensolver(H, eigvals, op, op_data, NULL, NULL, pre_op, pre_op_data, maxwell_parity_constraint,
                (void *)mdata, W, NWORK, error_tol, &num_iters, EIGS_DEFAULT_FLAGS);

    if (do_target) eigensolver_get_eigenvals(H, eigvals, maxwell_operator, mdata, W[0], W[1]);

    printf("Solved for eigenvectors after %d iterations.\n", num_iters);
    printf("%15s%15s%15s%15s\n", "eigenval", "frequency", "exact freq.", "error");
    for (i = 0; i < num_bands; ++i) {
      double err;
      real freq = sqrt(eigvals[i]);
      real exact_freq = bragg_omega(freq, kvector[0], sqrt(ed.eps_high), ed.eps_high_x,
                                    sqrt(ed.eps_low), 1.0 - ed.eps_high_x, 1.0e-7);
  ...
  if (!stop1) {
    ...
    evectmatrix_copy(H, Hstart);
    eigensolver(H, eigvals, op, op_data, NULL, NULL, NULL, NULL, maxwell_parity_constraint,
                (void *)mdata, W, NWORK, error_tol, &num_iters, EIGS_DEFAULT_FLAGS);
    ...
    evectmatrix_copy(H, Hstart);
    eigensolver(H, eigvals, op, op_data, NULL, NULL, pre_op, pre_op_data, maxwell_parity_constraint,
                (void *)mdata, W, NWORK - 1, error_tol, &num_iters, EIGS_DEFAULT_FLAGS);
