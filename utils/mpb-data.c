			    ...
			    int pick_nearest, int transpose)
{
     ...
     for (dim = 0; dim < 3; ++dim)
	  for (ri = 0; ri < 2; ++ri) {
	       ...
	       if (!dim && !ri) {
		    rank = rnk;
		    for (i = 0; i < 3; ++i) in_dims[i] = dims[i];
	       }
	       else {
		    if (rank != rnk || in_dims[0] != dims[0] ||
			in_dims[1] != dims[1] || in_dims[2] != dims[2])
			 goto bad;
	       }
	  }

     if (verbose)
	  printf("Found complex vector dataset...\n");
     ...
     if (verbose)
	  printf("Rotating vectors by matrix [ %10f%10f%10f\n"
		 "                             %10f%10f%10f\n"
		 "                             %10f%10f%10f ]\n",
		 cart_map.c0.x, cart_map.c1.x, cart_map.c2.x,
		 cart_map.c0.y, cart_map.c1.y, cart_map.c2.y,
