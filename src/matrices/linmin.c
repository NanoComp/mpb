...
extern int dcsrch(double *stp, double *f, double *g, double *ftol, double *gtol, double *xtol,
...
double linmin(double *converged_f, double *converged_df, double x_guess, double f_0, double df_0,
              double f_tol, double df_tol, double x_tol, double x_min, double x_max, linmin_func f,
  ...
  if (df_0 > 0) { /* minimum lies for negative x; transform to f(-x) */
    ...
    d.f = f;
    d.f_data = f_data;
    x = linmin(converged_f, converged_df, -x_guess, f_0, -df_0, f_tol, df_tol, x_tol, -x_min,
               -x_max, reverse_func, &d, verbose);
    ...
    *converged_df = df_0;
    ...
    x = x_guess;
    f_x = f_0;
    df_x = df_0; /* initially, pass in f and df at x=0 */
    dcsrch(&x, &f_x, &df_x, &f_tol, &df_tol, &x_tol, task, &x_min, &x_max, isave, dsave);
