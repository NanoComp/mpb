...
void sqmatrix_assert_hermitian(sqmatrix A) {
  ...
  for (i = 0; i < p; ++i) {
    err = max2(err, (SCALAR_IM(A.data[i * p + i]) * SCALAR_IM(A.data[i * p + i])) / maxsq);
    for (j = i + 1; j < p; ++j) {
      scalar x;
...
void sqmatrix_symmetrize(sqmatrix Asym, sqmatrix A) {
  ...
  for (i = 0; i < A.p; ++i)
    for (j = 0; j < A.p; ++j) {
      ...
      ASSIGN_SCALAR(Asym.data[ij], 0.5 * (SCALAR_RE(A.data[ij]) + SCALAR_RE(A.data[ji])),
                    0.5 * (SCALAR_IM(A.data[ij]) - SCALAR_IM(A.data[ji])));
    }
...
void sqmatrix_AeBC(sqmatrix A, sqmatrix B, short bdagger, sqmatrix C, short cdagger) {
  CHECK(A.p == B.p && A.p == C.p, "matrices not conformant");

  blasglue_gemm(bdagger ? 'C' : 'N', cdagger ? 'C' : 'N', A.p, A.p, A.p, 1.0, B.data, B.p, C.data,
                C.p, 0.0, A.data, A.p);
}

/* A += a B * C.  bdagger, cdagger are as for sqmatrix_AeBC, above. */
void sqmatrix_ApaBC(sqmatrix A, real a, sqmatrix B, short bdagger, sqmatrix C, short cdagger) {
  CHECK(A.p == B.p && A.p == C.p, "matrices not conformant");

  blasglue_gemm(bdagger ? 'C' : 'N', cdagger ? 'C' : 'N', A.p, A.p, A.p, a, B.data, B.p, C.data,
                C.p, 1.0, A.data, A.p);
}
...
void sqmatrix_aApbB(real a, sqmatrix A, real b, sqmatrix B) {
  ...
  if (a != 1.0) blasglue_rscal(A.p * A.p, a, A.data, 1);
...
int sqmatrix_invert(sqmatrix U, short positive_definite, sqmatrix Work) {
  int i, j;
  ...
  if (positive_definite) {
    ...
    /* factorize U: */
    if (!lapackglue_hetrf('U', U.p, U.data, U.p, ipiv, Work.data, Work.p * Work.p)) return 0;
    /* Compute 1/U (upper half only) */
    if (!lapackglue_hetri('U', U.p, U.data, U.p, ipiv, Work.data)) return 0;
...
void sqmatrix_gen_eigensolve(sqmatrix U, sqmatrix B, real *eigenvals, sqmatrix W) {
  ...
  if (B.data) {
    ...
    lapackglue_hegv(1, 'V', 'U', U.p, U.data, B.p, B.data, U.p, eigenvals, morework, nwork, work);
  }
  else {
    lapackglue_heev('V', 'U', U.p, U.data, U.p, eigenvals, morework, nwork, work);
  }
...
void sqmatrix_eigenvalues(sqmatrix A, scalar_complex *eigvals) {
  ...
  lapackglue_geev('N', 'N', A.p, B.data, A.p, eigvals, NULL, 1, NULL, 1, &work1, -1, rwork);
  lwork = (int)(SCALAR_RE(work1) + 0.5);
  CHK_MALLOC(work, scalar, lwork);
  lapackglue_geev('N', 'N', A.p, B.data, A.p, eigvals, NULL, 1, NULL, 1, work, lwork, rwork);
  free(work);
  free(rwork);
  destroy_sqmatrix(B);
}
