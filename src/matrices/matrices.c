...
evectmatrix create_evectmatrix(int N, int c, int p, int localN, int Nstart, int allocN) {
  evectmatrix X;

  CHECK(localN <= N && allocN >= localN && Nstart < N, "invalid N arguments");
  ...
  if (allocN > 0) { CHK_MALLOC(X.data, scalar, allocN * c * p); }
  else
    X.data = NULL;
...
void destroy_evectmatrix(evectmatrix X) { free(X.data); }

sqmatrix create_sqmatrix(int p) {
  sqmatrix X;

  X.alloc_p = X.p = p;
  if (p > 0) { CHK_MALLOC(X.data, scalar, p * p); }
  else
    X.data = (scalar *)NULL;
  return X;
}

void destroy_sqmatrix(sqmatrix X) { free(X.data); }
...
void matrix_XpaY_diag(scalar *X, real a, scalar *Y, scalar *diag, int n, int p) {
  int i, j;
  ...
  for (i = 0; i < n; ++i) {
    for (j = 0; j < p; ++j) {
      ...
      ASSIGN_SCALAR(X[i * p + j], SCALAR_RE(X[i * p + j]) + a * SCALAR_RE(c),
...
void matrix_XpaY_diag_real(scalar *X, real a, scalar *Y, real *diag, int n, int p) {
  int i, j;

  for (i = 0; i < n; ++i) {
    for (j = 0; j < p; ++j) {
      real d = a * diag[j];
      ASSIGN_SCALAR(X[i * p + j], SCALAR_RE(X[i * p + j]) + d * SCALAR_RE(Y[i * p + j]),
...
void matrix_X_diag_real_pY_diag_real(scalar *X, real *diag1, scalar *Y, real *diag2, int n, int p) {
  int i, j;

  for (i = 0; i < n; ++i) {
    for (j = 0; j < p; ++j) {
      real d1 = diag1[j], d2 = diag2[j];
      ASSIGN_SCALAR(X[i * p + j], d1 * SCALAR_RE(X[i * p + j]) + d2 * SCALAR_RE(Y[i * p + j]),
                    d1 * SCALAR_IM(X[i * p + j]) + d2 * SCALAR_IM(Y[i * p + j]));
    }
  }
}
