...
void fieldio_write_complex_field(scalar_complex *field, int rank, const int dims[3],
                                 const int local_dims[3], const int start[3], int which_component,
                                 int num_components, const real *kvector, matrixio_id file_id,
                                 int append, matrixio_id data_id[]) {
      ...
      for (ri_part = 0; ri_part < 2; ++ri_part) {
        ...
        if (!append)
          data_id[component * 2 + ri_part] =
              matrixio_create_dataset(file_id, name, NULL, rank, dims);

        matrixio_write_real_data(data_id[component * 2 + ri_part], local_dims, start,
                                 2 * num_components,
                                 ri_part ? &field[component].im : &field[component].re);
      }
}

void fieldio_write_real_vals(real *vals, int rank, const int dims[3], const int local_dims[3],
                             const int start[3], matrixio_id file_id, int append,
                             const char *dataname, matrixio_id *data_id) {
  rank = dims[2] == 1 ? (dims[1] == 1 ? 1 : 2) : 3;
