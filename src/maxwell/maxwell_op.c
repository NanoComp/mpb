...
static void assign_t2c(scalar *a, const k_data k, const scalar *v, int vstride) {
  scalar v0 = v[0], v1 = v[vstride];

  ASSIGN_SCALAR(a[0], SCALAR_RE(v0) * k.mx + SCALAR_RE(v1) * k.nx,
                SCALAR_IM(v0) * k.mx + SCALAR_IM(v1) * k.nx);
  ASSIGN_SCALAR(a[1], SCALAR_RE(v0) * k.my + SCALAR_RE(v1) * k.ny,
                SCALAR_IM(v0) * k.my + SCALAR_IM(v1) * k.ny);
  ASSIGN_SCALAR(a[2], SCALAR_RE(v0) * k.mz + SCALAR_RE(v1) * k.nz,
                SCALAR_IM(v0) * k.mz + SCALAR_IM(v1) * k.nz);
}

/* project from cartesian to transverse coordinates (inverse of assign_t2c) */
static void project_c2t(scalar *v, int vstride, const k_data k, const scalar *a, real scale) {
...
/* assign a = k x v (cross product), going from transverse to
   ...
   k.m and k.n. */
static void assign_cross_t2c(scalar *a, const k_data k, const scalar *v, int vstride) {
  scalar v0 = v[0], v1 = v[vstride];

  /* Note that k x m = |k| n, k x n = - |k| m.  Therefore,
     k x v = k x (v0 m + v1 n) = (v0 n - v1 m) * |k|. */

  ASSIGN_SCALAR(a[0], (SCALAR_RE(v0) * k.nx - SCALAR_RE(v1) * k.mx) * k.kmag,
                (SCALAR_IM(v0) * k.nx - SCALAR_IM(v1) * k.mx) * k.kmag);
  ASSIGN_SCALAR(a[1], (SCALAR_RE(v0) * k.ny - SCALAR_RE(v1) * k.my) * k.kmag,
                (SCALAR_IM(v0) * k.ny - SCALAR_IM(v1) * k.my) * k.kmag);
  ASSIGN_SCALAR(a[2], (SCALAR_RE(v0) * k.nz - SCALAR_RE(v1) * k.mz) * k.kmag,
...
/* assign v = scale * k x a (cross product), going from cartesian to
   ...
   cartesian coordinates.  (v[0],v[vstride]) is in the transverse basis of
   k.m and k.n. */
static void assign_cross_c2t(scalar *v, int vstride, const k_data k, const scalar *a, real scale) {
...
static void assign_ucross_t2c(scalar *a, const real u[3], const k_data k, const scalar *v,
                              int vstride) {
  scalar v0 = v[0], v1 = v[vstride];
  real vx_r, vy_r, vz_r;
  ...
  ASSIGN_SCALAR(a[0], u[1] * vz_r - u[2] * vy_r, u[1] * vz_i - u[2] * vy_i);
  ASSIGN_SCALAR(a[1], u[2] * vx_r - u[0] * vz_r, u[2] * vx_i - u[0] * vz_i);
  ASSIGN_SCALAR(a[2], u[0] * vy_r - u[1] * vx_r, u[0] * vy_i - u[1] * vx_i);
}

/**************************************************************************/

void maxwell_compute_fft(int dir, maxwell_data *d, scalar *array_in, scalar *array_out, int howmany,
                         int stride, int dist) {
  ...
  for (ip = 0; ip < d->nplans && (howmany != d->plans_howmany[ip] ||
                                  stride != d->plans_stride[ip] || dist != d->plans_dist[ip]);
       ++ip)
    ;
  if (ip < d->nplans) {
    ...
    plan = FFTW(mpi_plan_many_dft)(3, np, howmany, FFTW_MPI_DEFAULT_BLOCK, FFTW_MPI_DEFAULT_BLOCK,
                                   carray_in, carray_out, mpb_comm, FFTW_BACKWARD,
                                   FFTW_ESTIMATE | FFTW_MPI_TRANSPOSED_IN);
    iplan = FFTW(mpi_plan_many_dft)(3, np, howmany, FFTW_MPI_DEFAULT_BLOCK, FFTW_MPI_DEFAULT_BLOCK,
                                    carray_in, carray_out, mpb_comm, FFTW_FORWARD,
                                    FFTW_ESTIMATE | FFTW_MPI_TRANSPOSED_OUT);
#else  /* !HAVE_MPI */
    plan = FFTW(plan_many_dft)(3, n, howmany, carray_in, 0, stride, dist, carray_out, 0, stride,
                               dist, FFTW_BACKWARD, FFTW_ESTIMATE);
    iplan = FFTW(plan_many_dft)(3, n, howmany, carray_in, 0, stride, dist, carray_out, 0, stride,
...
#ifdef HAVE_MPI
      CHECK(stride == howmany && dist == 1, "bug: unsupported stride/dist");
      plan = FFTW(mpi_plan_many_dft_c2r)(rnk, np, howmany, FFTW_MPI_DEFAULT_BLOCK,
                                         FFTW_MPI_DEFAULT_BLOCK, carray_in, rarray_out, mpb_comm,
                                         FFTW_ESTIMATE | FFTW_MPI_TRANSPOSED_IN);
      iplan = FFTW(mpi_plan_many_dft_r2c)(rnk, np, howmany, FFTW_MPI_DEFAULT_BLOCK,
                                          FFTW_MPI_DEFAULT_BLOCK, rarray_in, carray_out, mpb_comm,
                                          FFTW_ESTIMATE | FFTW_MPI_TRANSPOSED_OUT);
#else  /* !HAVE_MPI */
      plan = FFTW(plan_many_dft_c2r)(rnk, n, howmany, carray_in, 0, stride, dist, rarray_out, nr,
                                     stride, dist, FFTW_ESTIMATE);
      iplan = FFTW(plan_many_dft_r2c)(rnk, n, howmany, rarray_in, nr, stride, dist, carray_out, 0,
                                      stride, dist, FFTW_ESTIMATE);
#endif /* !HAVE_MPI */
    }
  ...
  fftwnd((fftplan)(dir < 0 ? d->plans[0] : d->iplans[0]), howmany, (fftw_complex *)array_in, stride,
         dist, 0, 0, 0);

#else /* HAVE_MPI */

  CHECK(stride == howmany && dist == 1, "weird strides and dists don't work with fftwnd_mpi");

  fftwnd_mpi((fftplan)(dir < 0 ? d->plans[0] : d->iplans[0]), howmany, (fftw_complex *)array_in,
             (fftw_complex *)NULL, FFTW_TRANSPOSED_ORDER);
  ...
  if (dir > 0)
    rfftwnd_real_to_complex((fftplan)(d->iplans[0]), howmany, (fftw_real *)array_in, stride, dist,
                            0, 0, 0);
  else
    rfftwnd_complex_to_real((fftplan)(d->plans[0]), howmany, (fftw_complex *)array_in, stride, dist,
                            0, 0, 0);

#else /* HAVE_MPI */

  CHECK(stride == howmany && dist == 1, "weird strides and dists don't work with rfftwnd_mpi");

  rfftwnd_mpi((fftplan)(dir < 0 ? d->plans[0] : d->iplans[0]), howmany, array_in, (scalar *)NULL,
...
void assign_symmatrix_vector(scalar_complex *newv, const symmetric_matrix matrix,
  ...
  {
    real dummy;
    dummy = CSCALAR_NORMSQR(newv[0]) + CSCALAR_NORMSQR(newv[1]) + CSCALAR_NORMSQR(newv[2]);
    CHECK(!BADNUM(dummy), "yikes, crazy number!");
  }
#endif
}
...
/* compute the D field in position space from Hin, which holds the H
   ...
   here. */
void maxwell_compute_d_from_H(maxwell_data *d, evectmatrix Hin, scalar_complex *dfield,
                              int cur_band_start, int cur_num_bands) {
  scalar *fft_data = (scalar *)dfield;
  scalar *fft_data_in = d->fft_data2 == d->fft_data
                            ? fft_data
                            : (fft_data == d->fft_data ? d->fft_data2 : d->fft_data);
  int i, j, b;
  ...
  for (i = 0; i < d->other_dims; ++i)
    for (j = 0; j < d->last_dim; ++j) {
      ...
      for (b = 0; b < cur_num_bands; ++b)
        assign_cross_t2c(&fft_data_in[3 * (ij2 * cur_num_bands + b)], cur_k,
                         &Hin.data[ij * 2 * Hin.p + b + cur_band_start], Hin.p);
    }

  /* now, convert to position space via FFT: */
  maxwell_compute_fft(+1, d, fft_data_in, fft_data, cur_num_bands * 3, cur_num_bands * 3, 1);
}
...
/* Compute E (output in dfield) from D (input in dfield); this amounts
   ...
   above. */
void maxwell_compute_e_from_d_(maxwell_data *d, scalar_complex *dfield, int cur_num_bands,
...
void maxwell_compute_e_from_d(maxwell_data *d, scalar_complex *dfield, int cur_num_bands) {
...
void maxwell_compute_H_from_e(maxwell_data *d, evectmatrix Hout, scalar_complex *efield,
                              int cur_band_start, int cur_num_bands, real scale) {
  scalar *fft_data = (scalar *)efield;
  scalar *fft_data_out = d->fft_data2 == d->fft_data
                             ? fft_data
                             : (fft_data == d->fft_data ? d->fft_data2 : d->fft_data);
  int i, j, b;
  ...
  maxwell_compute_fft(-1, d, fft_data, fft_data_out, cur_num_bands * 3, cur_num_bands * 3, 1);
  ...
  for (i = 0; i < d->other_dims; ++i)
    for (j = 0; j < d->last_dim; ++j) {
      ...
      for (b = 0; b < cur_num_bands; ++b)
        assign_cross_c2t(&Hout.data[ij * 2 * Hout.p + b + cur_band_start], Hout.p, cur_k,
                         &fft_data_out[3 * (ij2 * cur_num_bands + b)], scale);
    }
}

/* Compute H field in position space from Hin.  Parameters and output
   formats are the same as for compute_d_from_H, above. */
void maxwell_compute_h_from_H(maxwell_data *d, evectmatrix Hin, scalar_complex *hfield,
                              int cur_band_start, int cur_num_bands) {
  ...
  for (i = 0; i < d->other_dims; ++i)
    for (j = 0; j < d->last_dim; ++j) {
      ...
      for (b = 0; b < cur_num_bands; ++b)
        assign_t2c(&fft_data_in[3 * (ij2 * cur_num_bands + b)], cur_k,
                   &Hin.data[ij * 2 * Hin.p + b + cur_band_start], Hin.p);
    }
...
void maxwell_compute_H_from_B(maxwell_data *d, evectmatrix Bin, evectmatrix Hout,
                              scalar_complex *hfield, int Bin_band_start, int Hout_band_start,
                              int cur_num_bands) {
  scalar *fft_data = (scalar *)hfield;
  scalar *fft_data_out = d->fft_data2 == d->fft_data
                             ? fft_data
  ...
  if (d->mu_inv == NULL) {
    if (Bin.data != Hout.data)
      evectmatrix_copy_slice(Hout, Bin, Hout_band_start, Bin_band_start, cur_num_bands);
    return;
  }
  ...
  for (i = 0; i < d->other_dims; ++i)
    for (j = 0; j < d->last_dim; ++j) {
      ...
      k_data cur_k = d->k_plus_G[ij];
      for (b = 0; b < cur_num_bands; ++b)
        project_c2t(&Hout.data[ij * 2 * Hout.p + b + Hout_band_start], Hout.p, cur_k,
                    &fft_data_out[3 * (ij2 * cur_num_bands + b)], scale);
    }
}
...
void maxwell_vectorfield_otherhalf(maxwell_data *d, scalar_complex *field, real phasex, real phasey,
                                   real phasez) {
...
#define ASSIGN_V(f, k, f2, k2)                                                                     \
  {                                                                                                \
    f[3 * (k) + 0] = f2[3 * (k2) + 0];                                                             \
    f[3 * (k) + 1] = f2[3 * (k2) + 1];                                                             \
    f[3 * (k) + 2] = f2[3 * (k2) + 2];                                                             \
  }
#define ASSIGN_VP(f, k, f2, k2, p)                                                                 \
  {                                                                                                \
    CASSIGN_MULT(f[3 * (k) + 0], f2[3 * (k2) + 0], p);                                             \
    CASSIGN_MULT(f[3 * (k) + 1], f2[3 * (k2) + 1], p);                                             \
    CASSIGN_MULT(f[3 * (k) + 2], f2[3 * (k2) + 2], p);                                             \
  }
#define ASSIGN_CV(f, k, f2, k2)                                                                    \
  {                                                                                                \
    CASSIGN_CONJ(f[3 * (k) + 0], f2[3 * (k2) + 0]);                                                \
...
/* as vectorfield_otherhalf, but operates on a complex scalar field
   ... ugh, copy & paste job */
void maxwell_cscalarfield_otherhalf(maxwell_data *d, scalar_complex *field, real phasex,
...
#define ASSIGN_V(f, k, f2, k2)                                                                     \
  { f[k] = f2[k2]; }
#define ASSIGN_VP(f, k, f2, k2, p)                                                                 \
  { CASSIGN_MULT(f[k], f2[k2], p); }
#define ASSIGN_CV(f, k, f2, k2)                                                                    \
  { CASSIGN_CONJ(f[k], f2[k2]); }
...
void maxwell_operator(evectmatrix Xin, evectmatrix Xout, void *data, int is_current_eigenvector,
                      evectmatrix Work) {
  ...
  for (cur_band_start = 0; cur_band_start < Xin.p; cur_band_start += d->num_fft_bands) {
    int cur_num_bands = MIN2(d->num_fft_bands, Xin.p - cur_band_start);

    if (d->mu_inv == NULL)
      maxwell_compute_d_from_H(d, Xin, cdata, cur_band_start, cur_num_bands);
    else {
      maxwell_compute_H_from_B(d, Xin, Xout, cdata, cur_band_start, cur_band_start, cur_num_bands);
      maxwell_compute_d_from_H(d, Xout, cdata, cur_band_start, cur_num_bands);
    }
    maxwell_compute_e_from_d(d, cdata, cur_num_bands);
    maxwell_compute_H_from_e(d, Xout, cdata, cur_band_start, cur_num_bands, scale);
    maxwell_compute_H_from_B(d, Xout, Xout, cdata, cur_band_start, cur_band_start, cur_num_bands);
  }
}
...
void maxwell_ucross_op(evectmatrix Xin, evectmatrix Xout, maxwell_data *d, const real u[3]) {
  ...
  for (cur_band_start = 0; cur_band_start < Xin.p; cur_band_start += d->num_fft_bands) {
    ...
    for (i = 0; i < d->other_dims; ++i)
      for (j = 0; j < d->last_dim; ++j) {
        ...
        for (b = 0; b < cur_num_bands; ++b)
          assign_ucross_t2c(&fft_data_in[3 * (ij2 * cur_num_bands + b)], u, cur_k,
                            &Xin.data[ij * 2 * Xin.p + b + cur_band_start], Xin.p);
      }
