...
void maxwell_parity_constraint(evectmatrix X, void *data) {
  ...
  if (d->parity & (EVEN_Z_PARITY | ODD_Z_PARITY)) maxwell_zparity_constraint(X, data);
  if (d->parity & (EVEN_Y_PARITY | ODD_Y_PARITY)) maxwell_yparity_constraint(X, data);
}
...
void maxwell_zparity_constraint(evectmatrix X, void *data) {
  maxwell_data *d = (maxwell_data *)data;
  int i, j, b, nxy, nz;
  int zparity = ((d->parity & EVEN_Z_PARITY) ? +1 : ((d->parity & ODD_Z_PARITY) ? -1 : 0));
  ...
  for (i = 0; i < nxy; ++i) {
    for (j = 0; 2 * j <= nz; ++j) {
      ...
      for (b = 0; b < X.p; ++b) {
        ...
        ASSIGN_SCALAR(X.data[(ij * 2) * X.p + b], 0.5 * (SCALAR_RE(u) + zparity * SCALAR_RE(u2)),
        ...
        ASSIGN_SCALAR(X.data[(ij2 * 2) * X.p + b], 0.5 * (SCALAR_RE(u2) + zparity * SCALAR_RE(u)),
  ...
  for (i = 0; i < nxy; ++i)
    for (j = 0; 2 * j <= nz; ++j) {
      ...
      for (b = 0; b < X.p; ++b) {
        ...
        v2 = X.data[(ij2 * 2 + 1) * X.p + b];
        zp_scratch[b] +=
            (ij == ij2 ? 1.0 : 2.0) * (SCALAR_RE(u) * SCALAR_RE(u2) + SCALAR_IM(u) * SCALAR_IM(u2) -
                                       SCALAR_RE(v) * SCALAR_RE(v2) - SCALAR_IM(v) * SCALAR_IM(v2));
        norm_scratch[b] +=
            (ij == ij2 ? 1.0 : 2.0) * (SCALAR_RE(u) * SCALAR_RE(u) + SCALAR_IM(u) * SCALAR_IM(u) +
                                       SCALAR_RE(v) * SCALAR_RE(v) + SCALAR_IM(v) * SCALAR_IM(v));
      }
    }

  mpi_allreduce(zp_scratch, zparity, X.p, double, MPI_DOUBLE, MPI_SUM, mpb_comm);
  mpi_allreduce(norm_scratch, zp_scratch, X.p, double, MPI_DOUBLE, MPI_SUM, mpb_comm);
  for (b = 0; b < X.p; ++b)
...
void maxwell_yparity_constraint(evectmatrix X, void *data) {
  ...
  int yparity = ((d->parity & EVEN_Y_PARITY) ? +1 : ((d->parity & ODD_Y_PARITY) ? -1 : 0));
    ...
    for (j = 0; 2 * j <= ny; ++j) {
      ...
      for (k = 0; k < nz; ++k) {
        ...
        for (b = 0; b < X.p; ++b) {
          ...
          ASSIGN_SCALAR(X.data[(ijk * 2) * X.p + b], 0.5 * (SCALAR_RE(u) - yparity * SCALAR_RE(u2)),
      ...
      for (k = 0; k < nz; ++k) {
        ...
        for (b = 0; b < X.p; ++b) {
          ...
          yp_scratch[b] += (ijk == ijk2 ? 1.0 : 2.0) *
                           (SCALAR_RE(v) * SCALAR_RE(v2) + SCALAR_IM(v) * SCALAR_IM(v2) -
                            SCALAR_RE(u) * SCALAR_RE(u2) - SCALAR_IM(u) * SCALAR_IM(u2));
          norm_scratch[b] += (ijk == ijk2 ? 1.0 : 2.0) *
                             (SCALAR_RE(v) * SCALAR_RE(v) + SCALAR_IM(v) * SCALAR_IM(v) +
                              SCALAR_RE(u) * SCALAR_RE(u) + SCALAR_IM(u) * SCALAR_IM(u));
        }
      }
    }
  }

  mpi_allreduce(yp_scratch, yparity, X.p, double, MPI_DOUBLE, MPI_SUM, mpb_comm);
  mpi_allreduce(norm_scratch, yp_scratch, X.p, double, MPI_DOUBLE, MPI_SUM, mpb_comm);
  for (b = 0; b < X.p; ++b)
...
int maxwell_zero_k_num_const_bands(evectmatrix X, maxwell_data *d) {
  ...
  if (d->parity & (ODD_Z_PARITY | EVEN_Y_PARITY)) m_band = 0;
  if (d->parity & (ODD_Y_PARITY | EVEN_Z_PARITY)) n_band = 0;

  num_const_bands = m_band + n_band;

  if (num_const_bands > X.p) num_const_bands = X.p;
...
void maxwell_zero_k_set_const_bands(evectmatrix X, maxwell_data *d) {
  ...
  if (X.Nstart > 0) return; /* DC frequency is not on this process */
