...
void get_dfield(int which_band) {
  if (!mdata) {
    mpi_one_fprintf(stderr, "init-params must be called before get-dfield!\n");
    return;
  }
  if (!kpoint_index) {
    mpi_one_fprintf(stderr, "solve-kpoint must be called before get-dfield!\n");
    return;
  }
  if (which_band < 1 || which_band > H.p) {
    mpi_one_fprintf(stderr, "must have 1 <= band index <= num_bands (%d)\n", H.p);
    return;
  }
     ...
     initially normalized to sum to nx*ny*nz.) */
  {
    ...
    if (freqs.items[which_band - 1] != 0.0) { scale = -1.0 / freqs.items[which_band - 1]; }
    else
      scale = -1.0; /* arbitrary */
...
void get_hfield(integer which_band) {
  if (!mdata) {
    mpi_one_fprintf(stderr, "init-params must be called before get-hfield!\n");
    return;
  }
  if (!kpoint_index) {
    mpi_one_fprintf(stderr, "solve-kpoint must be called before get-hfield!\n");
    return;
  }
  if (which_band < 1 || which_band > H.p) {
    mpi_one_fprintf(stderr, "must have 1 <= band index <= num_bands (%d)\n", H.p);
...
void get_bfield(integer which_band) {
  if (!mdata) {
    mpi_one_fprintf(stderr, "init-params must be called before get-bfield!\n");
    return;
  }
  if (!kpoint_index) {
    mpi_one_fprintf(stderr, "solve-kpoint must be called before get-bfield!\n");
    return;
  }
  if (which_band < 1 || which_band > H.p) {
    mpi_one_fprintf(stderr, "must have 1 <= band index <= num_bands (%d)\n", H.p);
...
double compute_field_energy_internal(real comp_sum[6]) {
  ...
  for (i = 0; i < N; ++i) {
    ...
    real comp_sqr0, comp_sqr1, comp_sqr2, comp_sqr3, comp_sqr4, comp_sqr5;
  ...
  mpi_allreduce_1(&energy_sum, real, SCALAR_MPI_TYPE, MPI_SUM, mpb_comm);
  mpi_allreduce(comp_sum2, comp_sum, 6, real, SCALAR_MPI_TYPE, MPI_SUM, mpb_comm);
...
number_list compute_field_energy(void) {
  ...
  mpi_one_printf("%c-energy-components:, %d, %d", curfield_type, kpoint_index, curfield_band);
  for (i = 0; i < 6; ++i) {
    comp_sum[i] /= (energy_sum == 0 ? 1 : energy_sum);
    if (i % 2 == 1) mpi_one_printf(", %g", comp_sum[i] + comp_sum[i - 1]);
  }
  mpi_one_printf("\n");
...
void fix_field_phase(void) {
  ...
  mpi_allreduce(sq_sum2, sq_sum, 2, real, SCALAR_MPI_TYPE, MPI_SUM, mpb_comm);
  ...
  mpi_allreduce_1(&maxabs, real, SCALAR_MPI_TYPE, MPI_MAX, mpb_comm);
  for (i = N - 1; i >= 0; --i) {
#ifdef SCALAR_COMPLEX
  ...
  for (i = 0; i < H.n; ++i) {
    ASSIGN_MULT(H.data[i * H.p + curfield_band - 1], H.data[i * H.p + curfield_band - 1], phase);
  }
}
...
static real get_val(int ix, int iy, int iz, int nx, int ny, int nz, int last_dim_size, real *data,
  ...
  if (local_iy >= 0 && local_iy < local_ny) {
    val = data[(((local_iy * nx) + ix) * nz + iz) *
               stride]; /* note transposition in x and y indices */
  }
...
static real interp_val(vector3 p, int nx, int ny, int nz, int last_dim_size, real *data, int stride,
                       int conjugate) {
  double ipart;
  real rx, ry, rz, dx, dy, dz, v;
  int x, y, z, x2, y2, z2;

  rx = modf(p.x / geometry_lattice.size.x + 0.5, &ipart);
  if (rx < 0) rx += 1;
  ry = modf(p.y / geometry_lattice.size.y + 0.5, &ipart);
  if (ry < 0) ry += 1;
  rz = modf(p.z / geometry_lattice.size.z + 0.5, &ipart);
  if (rz < 0) rz += 1;
...
static scalar_complex interp_cval(vector3 p, int nx, int ny, int nz, int last_dim_size, real *data,
                                  int stride) {
  scalar_complex cval;
...
cvector3 get_field_point(vector3 p) {
  ...
  if (curfield_type != 'v') {
    double phase_phi = TWOPI * (cur_kvector.x * (p.x / geometry_lattice.size.x) +
...
number rscalar_field_get_point(SCM fo, vector3 p) {
  ...
  CHECK(f->type == RSCALAR_FIELD_SMOB, "invalid argument to rscalar-field-get-point");
...
cvector3 cvector_field_get_point_bloch(SCM fo, vector3 p) {
  ...
  cvector3 F;
  field_smob *f = assert_field_smob(fo);
  CHECK(f->type == CVECTOR_FIELD_SMOB, "invalid argument to cvector-field-get-point");
...
cnumber cscalar_field_get_point_bloch(SCM fo, vector3 p) {
  field_smob *f = assert_field_smob(fo);
  CHECK(f->type == CSCALAR_FIELD_SMOB, "invalid argument to cscalar-field-get-point-bloch");
...
cnumber cscalar_field_get_point(SCM fo, vector3 p) {
  scalar_complex s;
  field_smob *f = assert_field_smob(fo);
  CHECK(f->type == CSCALAR_FIELD_SMOB, "invalid argument to cscalar-field-get-point");
  ...
  for (i = 0; i < N; ++i) {
    ...
    if (epsilon >= eps_low && epsilon <= eps_high) {
      ...
      /* most points are counted twice, by rfftw output symmetry: */
      {
        ...
        last_index = i % last_dim_stored;
#endif
        if (last_index != 0 && 2 * last_index != last_dim) energy_sum += energy[i];
      }
#endif
    }
  }
...
static char *fix_fname(const char *fname, const char *prefix, maxwell_data *d, int parity_suffix) {
  char *s;
  CHK_MALLOC(s, char, (fname ? strlen(fname) : 0) + (prefix ? strlen(prefix) : 0) + 20);
...
static void output_scalarfield(real *vals, const int dims[3], const int local_dims[3],
                               const int start[3], matrixio_id file_id, const char *dataname,
                               int last_dim_index, int last_dim_start, int last_dim_size,
                               int first_dim_start, int first_dim_size, int write_start0_special) {
...
#ifndef SCALAR_COMPLEX
  {
    ...
    if (write_start0_special) {
      ...
      fieldio_write_real_vals(vals + local_dims_new[1] * local_dims_new[2], 3, dims, local_dims_new,
                              start_new, file_id, 1, dataname, &data_id);
      local_dims_new[0] = 1;
      start_new[0] = 0;
      fieldio_write_real_vals(vals, 3, dims, local_dims_new, start_new, file_id, 1, dataname,
                              &data_id);
    }
    else {
      fieldio_write_real_vals(vals, 3, dims, local_dims_new, start_new, file_id, 1, dataname,
  ...
  if (data_id.id >= 0) matrixio_close_dataset(data_id);
}
...
void output_field_to_file(integer which_component, string filename_prefix) {
  ...
  if (!curfield) {
    mpi_one_fprintf(stderr, "fields, energy dens., or epsilon must be loaded first.\n");
    return;
  }
  ...
  if (dims[2] == 1) {
    ...
    if (start[0] == 0) --first_dim_size; /* DC frequency is not in other half */
...
#endif /* ! SCALAR_COMPLEX */
  output_k[0] =
      R[1][0] * mdata->current_k[0] + R[1][1] * mdata->current_k[1] + R[1][2] * mdata->current_k[2];
  output_k[1] =
      R[0][0] * mdata->current_k[0] + R[0][1] * mdata->current_k[1] + R[0][2] * mdata->current_k[2];
  output_k[2] =
      R[2][0] * mdata->current_k[0] + R[2][1] * mdata->current_k[1] + R[2][2] * mdata->current_k[2];
  output_R[0][0] = R[1][0];
  output_R[0][1] = R[1][1];
  output_R[0][2] = R[1][2];
  output_R[1][0] = R[0][0];
  ...
  if (strchr("dhbecv", curfield_type)) { /* outputting vector field */
    ...
    free(fname2);
    fieldio_write_complex_field(curfield, 3, dims, local_dims, start, which_component, 3, output_k,
                                file_id, 0, data_id);
    ...
    if (write_start0_special) {
      ...
      fieldio_write_complex_field(curfield + 3 * local_dims[1] * local_dims[2], 3, dims, local_dims,
                                  start, which_component, 3, NULL, file_id, 1, data_id);
      local_dims[0] = 1;
      start[0] = 0;
      fieldio_write_complex_field(curfield, 3, dims, local_dims, start, which_component, 3, NULL,
                                  file_id, 1, data_id);
    }
    else {
      fieldio_write_complex_field(curfield, 3, dims, local_dims, start, which_component, 3, NULL,
    ...
    for (i = 0; i < 6; ++i)
      if (data_id[i].id >= 0) matrixio_close_dataset(data_id[i]);
    ...
    file_id = matrixio_create(fname2);
    free(fname2);
    fieldio_write_complex_field(curfield, 3, dims, local_dims, start, which_component, 1, output_k,
                                file_id, 0, data_id);
    ...
    if (write_start0_special) {
      /* The conjugated array half may be discontiguous.
         First, write the part not containing start[0], and
         then write the start[0] slab. */
      fieldio_write_complex_field(curfield + local_dims[1] * local_dims[2], 3, dims, local_dims,
                                  start, which_component, 1, NULL, file_id, 1, data_id);
      local_dims[0] = 1;
      start[0] = 0;
    ...
    if (curfield_type == 'n') {
      ...
      sprintf(description, "%c field energy density, kpoint %d, band %d, freq=%g", curfield_type,
    ...
    output_scalarfield((real *)curfield, dims, local_dims, start, file_id, "data", last_dim_index,
                       last_dim_start, last_dim_size, first_dim_start, first_dim_size,
      ...
      for (inv = 0; inv < 2; ++inv)
        for (c1 = 0; c1 < 3; ++c1)
          for (c2 = c1; c2 < 3; ++c2) {
            ...
            sprintf(dataname, "%s.%c%c", inv ? "epsilon_inverse" : "epsilon", c1 + 'x', c2 + 'x');
  ...
  LOOP_XYZ(mdata) {
    ...
    for (n = objects.num_items - 1; n >= 0; --n)
      if (point_in_periodic_fixed_objectp(p, objects.items[n])) {
        if (objects.items[n].material.which_subclass == MATERIAL_TYPE_SELF)
...
#ifndef SCALAR_COMPLEX
        {
          ...
          last_index = j;
#endif
          if (last_index != 0 && 2 * last_index != last_dim) energy_sum += energy[xyz_index];
        }
#endif
        break;
      }
...
cnumber compute_field_integral(function f) {
  ...
  LOOP_XYZ(mdata) {
    ...
    p.z = i3 * s3 - c3;
    if (integrate_energy) {
      integral.re += ctl_convert_number_to_c(
          gh_call3(f, ctl_convert_number_to_scm(energy[xyz_index]),
                   ctl_convert_number_to_scm(epsilon), ctl_convert_vector3_to_scm(p)));
    }
    else {
      cvector3 F;
      double phase_phi;
      ...
      phase_phi = TWOPI * (kvector.x * (p.x / geometry_lattice.size.x) +
      ...
      integrand = ctl_convert_cnumber_to_c(gh_call3(f, ctl_convert_cvector3_to_scm(F),
