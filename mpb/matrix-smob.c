...
static SCM evectmatrix_p(SCM obj) { return ctl_convert_boolean_to_scm(EVECTMATRIX_P(obj)); }
...
static SCM sqmatrix_p(SCM obj) { return ctl_convert_boolean_to_scm(SQMATRIX_P(obj)); }
...
SCM evectmatrix_slice2scm(evectmatrix m, int p_start, int p) {
  SCM obj;
  evectmatrix *mp;
  CHECK(p_start >= 0 && p_start + p <= m.p && p >= 0, "invalid arguments in evectmatrix_slice2scm");
  CHK_MALLOC(mp, evectmatrix, 1);
...
cnumber sqmatrix_ref(SCM mo, integer i, integer j) {
  sqmatrix *m = assert_sqmatrix_smob(mo);
  cnumber c;
  CHECK(m && i >= 0 && j >= 0 && i < m->p && j < m->p, "invalid arguments to sqmatrix-ref");
...
void sqmatrix_set(SCM mo, integer i, integer j, cnumber c) {
  sqmatrix *m = assert_sqmatrix_smob(mo);
  CHECK(m && i >= 0 && j >= 0 && i < m->p && j < m->p, "invalid arguments to sqmatrix-set");
...
SCM dot_eigenvectors(SCM mo, integer b_start) {
  ...
  if (mdata->mu_inv == NULL) {
    ...
    for (ib = b_start - 1; ib < final_band; ib += W[0].alloc_p) {
      ...
      maxwell_compute_H_from_B(mdata, H, W[0], (scalar_complex *)mdata->fft_data, ib, 0, W[0].p);

      evectmatrix_XtY_slice2(U, *m, W[0], 0, 0, m->p, W[0].p, ib - (b_start - 1), S1, S2);
    }
