...
#define REGISTER_SMOBFUNS(T)                                                                       \
  do {                                                                                             \
    scm_tc16_smob_##T = scm_newsmob(&T##_smobfuns);                                                \
  } while (0)
#define MAKE_SMOBFUNS(T) static scm_smobfuns T##_smobfuns = {&mark_##T, &free_##T, &print_##T, 0}
#endif /* ! HAVE_SCM_MAKE_SMOB_TYPE */

#ifdef HAVE_SCM_NEWSMOB
#define NEWCELL_SMOB(ANSWER, T, PSMOB) SCM_NEWSMOB(ANSWER, scm_tc16_smob_##T, PSMOB)
#else
...
#else
#define T_SMOB_P(T, X) (SCM_NIMP(X) && SCM_TYP16(X) == (scm_tc16_smob_##T))
#endif
