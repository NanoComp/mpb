...
real linear_interpolate(real rx, real ry, real rz, real *data, int nx, int ny, int nz, int stride) {
  int x, y, z, x2, y2, z2;
  real dx, dy, dz;
...
static void epsilon_file_func(symmetric_matrix *eps, symmetric_matrix *eps_inv, const real r[3],
                              void *edata) {
...
void get_epsilon_file_func(const char *fname, maxwell_dielectric_function *func, void **func_data) {
  if (fname && fname[0]) {
    char *eps_fname;
    matrixio_id file_id;
    epsilon_file_data *d;
    ...
    d->data = matrixio_read_real_data(file_id, NULL, &rank, dims, 0, 0, 0, NULL);
