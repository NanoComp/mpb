...
static SCM field_p(SCM obj) { return ctl_convert_boolean_to_scm(FIELD_P(obj)); }

static SCM rscalar_field_p(SCM obj) { return ctl_convert_boolean_to_scm(RSCALAR_FIELD_P(obj)); }

static SCM cscalar_field_p(SCM obj) { return ctl_convert_boolean_to_scm(CSCALAR_FIELD_P(obj)); }

static SCM cvector_field_p(SCM obj) { return ctl_convert_boolean_to_scm(CVECTOR_FIELD_P(obj)); }
...
static int print_field_smob(SCM obj, SCM port, scm_print_state *pstate) {
  ...
  scm_puts(buf, port);
  switch (pf->type) {
    case RSCALAR_FIELD_SMOB: scm_puts(" real scalar field", port); break;
    case CSCALAR_FIELD_SMOB: scm_puts(" complex scalar field", port); break;
    case CVECTOR_FIELD_SMOB: scm_puts(" complex vector field", port); break;
  }
  if (pf->local_ny < pf->ny) {
    sprintf(buf, ", y=%d-%d local", pf->local_y_start, pf->local_y_start + pf->local_ny - 1);
    scm_puts(buf, port);
  }
  scm_putc('>', port);
  return 1;
...
field_smob *update_curfield_smob(void) {
  ...
  if (strchr("dhbecv", curfield_type)) { /* complex vector field */
    ...
    curfield_smob.f.rs = (real *)curfield;
    if (!curfield_smob.f.rs) curfield_smob.f.rs = (real *)mdata->fft_data;
    return 0;
  }
...
SCM field_make(SCM f0) {
  field_smob *pf0 = assert_field_smob(f0);
  switch (pf0->type) {
    case RSCALAR_FIELD_SMOB: return rscalar_field_make(f0);
    case CSCALAR_FIELD_SMOB: return cscalar_field_make(f0);
    case CVECTOR_FIELD_SMOB: return cvector_field_make(f0);
  }
  scm_remember_upto_here_1(f0);
...
#define EQF(field) (f1->field == f2->field)
  return (EQF(nx) && EQF(ny) && EQF(nz) && EQF(N) && EQF(local_ny) && EQF(local_y_start) &&
...
static void field_set(field_smob *fd, field_smob *fs) {
  ...
  CHECK(fd->type == fs->type && fields_conform(fd, fs), "fields for field-set! must conform");
  switch (fs->type) {
...
void field_load(SCM src) {
  ...
  CHECK(fields_conform(fs, &curfield_smob), "argument for field-load must conform to current size");
...
void field_mapLB(SCM dest, function f, SCM_list src) {
  ...
  for (j = 0; j < src.num_items; ++j) {
    ...
    CHECK(fields_conform(pd, ps[j]), "fields for field-map! must conform");
  }
  for (i = 0; i < pd->N; ++i) {
    ...
    for (j = src.num_items - 1; j >= 0; --j) {
      SCM item = SCM_EOL;
      switch (ps[j]->type) {
        case RSCALAR_FIELD_SMOB: item = ctl_convert_number_to_scm(ps[j]->f.rs[i]); break;
        case CSCALAR_FIELD_SMOB: item = cnumber2scm(cscalar2cnumber(ps[j]->f.cs[i])); break;
        case CVECTOR_FIELD_SMOB: item = cvector32scm(cscalar32cvector3(ps[j]->f.cv + 3 * i)); break;
      }
      arg_list = gh_cons(item, arg_list);
    }
    result = gh_apply(f, arg_list);
    switch (pd->type) {
      case RSCALAR_FIELD_SMOB: pd->f.rs[i] = ctl_convert_number_to_c(result); break;
      case CSCALAR_FIELD_SMOB: pd->f.cs[i] = cnumber2cscalar(scm2cnumber(result)); break;
      case CVECTOR_FIELD_SMOB: cvector32cscalar3(pd->f.cv + 3 * i, scm2cvector3(result)); break;
    }
  }
  if (src.num_items == 1 && ps[0]->type == pd->type)
    ...
    switch (pd->type) {
      case RSCALAR_FIELD_SMOB: pd->type_char = 'R'; break;
      case CSCALAR_FIELD_SMOB: pd->type_char = 'C'; break;
      case CVECTOR_FIELD_SMOB: pd->type_char = 'c'; break;
    }
  free(ps);
  update_curfield(pd);
...
cnumber integrate_fieldL(function f, SCM_list fields) {
  ...
  for (ifield = 0; ifield < fields.num_items; ++ifield) {
    ...
    CHECK(fields_conform(pf[0], pf[ifield]), "fields for integrate-fields must conform");
  }
          ...
          for (ifield = fields.num_items - 1; ifield >= 0; --ifield) {
            ...
            switch (pf[ifield]->type) {
              case RSCALAR_FIELD_SMOB:
                ...
                break;
              case CSCALAR_FIELD_SMOB:
                item = cnumber2scm(cscalar2cnumber(pf[ifield]->f.cs[index]));
                break;
              case CVECTOR_FIELD_SMOB:
                item = cvector32scm(cscalar32cvector3(pf[ifield]->f.cv + 3 * index));
                break;
            }
            arg_list = gh_cons(item, arg_list);
          }
            ...
            if (last_index != 0 && 2 * last_index != last_dim) {
              ...
              for (ifield = fields.num_items - 1; ifield >= 0; --ifield) {
                ...
                switch (pf[ifield]->type) {
                  ...
                  case CVECTOR_FIELD_SMOB:
                    item = cvector32scm(
                        cvector3_conj(cscalar32cvector3(pf[ifield]->f.cv + 3 * index)));
                    break;
                }
                arg_list = gh_cons(item, arg_list);
              }
