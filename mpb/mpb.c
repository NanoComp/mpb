...
#if defined(HAVE_MPI) &&                                                                           \
    (defined(HAVE_LIBFFTW3F_MPI) || defined(HAVE_LIBFFTW3L_MPI) || defined(HAVE_LIBFFTW3_MPI))
...
void ctl_start_hook(int *argc, char ***argv) {
...
#ifdef USE_OPENMP
  {
    ...
    int i, nthread = senthread ? (atoi(senthread) > 0 ? atoi(senthread) : 1) : 1;
    for (i = 0; i < *argc; ++i)
      if (!strncasecmp("--nthread=", (*argv)[i], 10)) {
        int j;
        CHECK((nthread = atoi((*argv)[i] + 10)) > 0, "invalid argument for --nthread=...");
        *argc -= 1;
        for (j = i; j < *argc; ++j)
          (*argv)[j] = (*argv)[j + 1];
      }
...
boolean mpi_is_masterp(void) { return mpi_is_master(); }
...
integer get_kpoint_index(void) { return kpoint_index; }

void set_kpoint_index(integer i) { kpoint_index = i; }
...
void set_parity(integer p) {
  ...
  if (!mdata) {
    mpi_one_fprintf(stderr, "init-params must be called before set-parity!\n");
    return;
  }

  if (p == -1) p = last_p < 0 ? NO_PARITY : last_p;
...
void init_params(integer p, boolean reset_fields) {
  ...
  if (target_freq != 0.0) mpi_one_printf("Target frequency is %g\n", target_freq);
...

  {
    ...
    if (true_rank < dimensions)
      ...
      mpi_one_fprintf(stderr, "WARNING: rank of grid is > dimensions.\n"
                              "         setting extra grid dims. to 1.\n");
      /* force extra dims to be 1: */
      if (dimensions <= 2) nz = 1;
      if (dimensions <= 1) ny = 1;
    }
  }
  ...
  if (mdata) { /* need to clean up from previous init_params call */
    if (nx == mdata->nx && ny == mdata->ny && nz == mdata->nz && block_size == Hblock.alloc_p &&
        num_bands == H.p && eigensolver_nwork + (mdata->mu_inv != NULL) == nwork_alloc)
  ...
  mdata = create_maxwell_data(nx, ny, nz, &local_N, &N_start, &alloc_N, block_size, NUM_FFT_BANDS);
  ...
  if (!have_old_fields) {
    ...
    H = create_evectmatrix(nx * ny * nz, 2, num_bands, local_N, N_start, alloc_N);
    nwork_alloc = eigensolver_nwork + (mdata->mu_inv != NULL);
    for (i = 0; i < nwork_alloc; ++i)
      W[i] = create_evectmatrix(nx * ny * nz, 2, block_size, local_N, N_start, alloc_N);
    if (block_size < num_bands)
      Hblock = create_evectmatrix(nx * ny * nz, 2, block_size, local_N, N_start, alloc_N);
    else
      Hblock = H;
    if (using_mup() && block_size < num_bands) {
      muinvH = create_evectmatrix(nx * ny * nz, 2, num_bands, local_N, N_start, alloc_N);
    }
    else {
      muinvH = H;
    }
  ...
  for (i = 0; i < k_points.num_items; ++i)
    mpi_one_printf("     (%g,%g,%g)\n", k_points.items[i].x, k_points.items[i].y,
                   k_points.items[i].z);
...

  {
    ...
    if (ierr == 1)
      mpi_one_fprintf(stderr, "ERROR: non positive-definite dielectric tensor\n");
    else if (ierr == 2)
      mpi_one_fprintf(stderr, "ERROR: dielectric tensor must not couple xy "
...
boolean using_mup(void) { return mdata && mdata->mu_inv != NULL; }
...
static void deflation_constraint(evectmatrix X, void *data) {
  ...
  mpi_allreduce(d->S2, d->S, d->p * X.p * SCALAR_NUMVALS, real, SCALAR_MPI_TYPE, MPI_SUM, mpb_comm);
...
void solve_kpoint(vector3 kvector) {
  ...
  if (!mdata) {
    mpi_one_fprintf(stderr, "init-params must be called before solve-kpoint!\n");
    return;
  }
  ...
  CHECK(mdata->parity == prev_parity, "k vector is incompatible with specified parity");

  CHK_MALLOC(eigvals, real, num_bands);

  flags = eigensolver_flags; /* ctl file input variable */
  if (verbose) flags |= EIGS_VERBOSE;
  ...
  for (ib = ib0; ib < num_bands; ib += Hblock.alloc_p) {
    ...
    if (mtdata) { /* solving for bands near a target frequency */
      ...
      if (eigensolver_davidsonp)
        eigensolver_davidson(Hblock, eigvals + ib, maxwell_target_operator, (void *)mtdata,
                             simple_preconditionerp ? maxwell_target_preconditioner
                                                    : maxwell_target_preconditioner2,
                             (void *)mtdata, evectconstraint_chain_func, (void *)constraints, W,
                             nwork_alloc, tolerance, &num_iters, flags, 0.0);
      else
        eigensolver(Hblock, eigvals + ib, maxwell_target_operator, (void *)mtdata, NULL, NULL,
                    simple_preconditionerp ? maxwell_target_preconditioner
                                           : maxwell_target_preconditioner2,
      ...
      eigensolver_get_eigenvals(Hblock, eigvals + ib, maxwell_operator, mdata, W[0], W[1]);
    }
    else {
      if (eigensolver_davidsonp) {
        CHECK(mdata->mu_inv == NULL, "Davidson doesn't handle mu");
        eigensolver_davidson(Hblock, eigvals + ib, maxwell_operator, (void *)mdata,
                             simple_preconditionerp ? maxwell_preconditioner
        ...
        eigensolver(Hblock, eigvals + ib, maxwell_operator, (void *)mdata,
                    ...
                    simple_preconditionerp ? maxwell_preconditioner : maxwell_preconditioner2,
  ...
  mpi_one_printf("%sfreqs:, %d, %g, %g, %g, %g", parity, kpoint_index, (double)k[0], (double)k[1],
                 (double)k[2], vector3_norm(matrix3x3_vector3_mult(Gm, kvector)));
  for (i = 0; i < num_bands; ++i) {
    freqs.items[i] = negative_epsilon_okp ? eigvals[i] : sqrt(eigvals[i]);
...
number_list compute_group_velocity_component(vector3 d) {
  ...
  for (ib = 0; ib < num_bands; ib += Hblock.alloc_p) {
    ...
    evectmatrix_XtY_diag_real(Hblock, W[0], gv_scratch, gv_scratch + group_v.num_items);
    {
      int ip;
      for (ip = 0; ip < Hblock.p; ++ip)
  ...
  for (i = 0; i < num_bands; ++i) {
    if (freqs.items[i] == 0)  /* v is undefined in this case */
      ...
      group_v.items[i] /= negative_epsilon_okp ? sqrt(fabs(freqs.items[i])) : freqs.items[i];
  }
...
number compute_1_group_velocity_component(vector3 d, integer b) {
  ...
  if (freqs.items[ib] == 0) /* v is undefined in this case */
    ...
    group_v /= negative_epsilon_okp ? sqrt(fabs(freqs.items[ib])) : freqs.items[ib];
