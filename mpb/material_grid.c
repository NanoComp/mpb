...
static double *material_grid_array(const material_grid *g) {
  ...
  return (double *)scm_array_handle_uniform_writable_elements(&cur_material_grid_array_handle);
#else
...
real material_grid_val(vector3 p, const material_grid *g) {
  ...
  CHECK(SCM_ARRAYP(g->matgrid), "bug: matgrid is not an array");
  val =
      linear_interpolate(p.x, p.y, p.z, material_grid_array(g), g->size.x, g->size.y, g->size.z, 1);
...
static int compatible_matgrids(const material_grid *mg, const material_type *m) {
  return (m->which_subclass == MATERIAL_GRID &&
          m->subclass.material_grid_data->material_grid_kind == mg->material_grid_kind &&
...
double matgrid_val(vector3 p, geom_box_tree tp, int oi, const material_grid *mg) {
  ...
  if (tp) {
    do {
      u = material_grid_val(to_geom_box_coords(p, &tp->objects[oi]),
                            tp->objects[oi].o->material.subclass.material_grid_data);
      if (u < umin) umin = u;
      uprod *= u;
      usum += u;
      ++matgrid_val_count;
      tp = geom_tree_search_next(p, tp, &oi);
    } while (tp && compatible_matgrids(mg, &tp->objects[oi].o->material));
  }
  if (!tp && compatible_matgrids(mg, &default_material)) {
    ...
    u = material_grid_val(p, default_material.subclass.material_grid_data);
    if (u < umin) umin = u;
    uprod *= u;
    usum += u;
    ++matgrid_val_count;
  }
  return (mg->material_grid_kind == U_MIN
              ? umin
              : (mg->material_grid_kind == U_PROD ? uprod : usum / matgrid_val_count));
}
  ...
  *ngrids = 0;
  for (i = 0; i < g.num_items; ++i)
    if (g.items[i].material.which_subclass == MATERIAL_GRID) {
      int j;
      for (j = 0; j < *ngrids; ++j)
        if (material_grid_equal(&grids[j], g.items[i].material.subclass.material_grid_data)) break;
  ...
  if (default_material.which_subclass == MATERIAL_GRID) {
    int j;
    for (j = 0; j < *ngrids; ++j)
      if (material_grid_equal(&grids[j], default_material.subclass.material_grid_data)) break;
    if (j == *ngrids) {
      if (j >= nalloc) {
        nalloc = nalloc * 2 + 1;
        grids = realloc(grids, sizeof(material_grid) * nalloc);
...
static void add_interpolate_weights(real rx, real ry, real rz, real *data, int nx, int ny, int nz,
                                    int stride, double scaleby, const real *udata, int ukind,
                                    double uval) {
  int x, y, z, x2, y2, z2;
  real dx, dy, dz, u;
...
static void material_grids_addgradient_point(double *v, vector3 p, double scalegrad,
  ...
  if ((kind = mg->material_grid_kind) == U_SUM) scalegrad /= matgrid_val_count;

  if (tp) {
    do {
      vector3 pb = to_geom_box_coords(p, &tp->objects[oi]);
      vector3 sz = tp->objects[oi].o->material.subclass.material_grid_data->size;
      double *vcur = v, *ucur;
      for (i = 0; i < ngrids; ++i) {
        if (material_grid_equal(grids + i, tp->objects[oi].o->material.subclass.material_grid_data))
          break;
        else
          vcur += (int)(grids[i].size.x * grids[i].size.y * grids[i].size.z);
      }
  ...
  if (!tp && compatible_matgrids(mg, &default_material)) {
    ...
    double *vcur = v, *ucur;
    for (i = 0; i < ngrids; ++i) {
      if (material_grid_equal(grids + i, default_material.subclass.material_grid_data))
        break;
      else
        vcur += (int)(grids[i].size.x * grids[i].size.y * grids[i].size.z);
    }
...
void material_grids_addgradient(double *v, double scalegrad, int band, const material_grid *grids,
  ...
  LOOP_XYZ(mdata) {
    ...
    material_grids_addgradient_point(v, p, Esqr[xyz_index] * scalegrad, grids, ngrids);
...
number material_grids_approx_gradient(vector3 kpoint, integer band, integer iu, number du) {
  int ngrids;
  material_grid *grids = get_material_grids(geometry, &ngrids);
  int i, ntot = material_grids_ntot(grids, ngrids);
  ...
  material_grids_set(u, grids, ngrids);
  reset_epsilon();
  mpi_one_printf("approxgrad: ntot=%d, u[%d] = %g -> f_%d = %g, u += %g -> f_%d = %g; df/du = %g "
                 "vs. analytic %g\n",
      ...
      for (k = 0; k < n3; ++k) {
...

        {
            ...
            do {
              ...
              for (ig = 0; ig < ngrids; ++ig) {
                if (material_grid_equal(grids + ig,
                  ...
                  vcur += (int)(grids[ig].size.x * grids[ig].size.y * grids[ig].size.z);
              }
              CHECK(ig < ngrids, "bug in material_grid_gradient_point");
              ucur = material_grid_array(grids + ig);
              add_interpolate_weights(pb.x, pb.y, pb.z, vcur, sz.x, sz.y, sz.z, 1, scalegrad, ucur,
                                      kind, uval);
          ...
          if (!tp && compatible_matgrids(mg, &default_material)) {
            ...
            double *vcur = v, *ucur;
            for (ig = 0; ig < ngrids; ++ig) {
              if (material_grid_equal(grids + ig, default_material.subclass.material_grid_data))
                break;
              else
                vcur += (int)(grids[ig].size.x * grids[ig].size.y * grids[ig].size.z);
            }
        ...
        gotmyv:
          mpi_one_printf("depsdu:, %g, %d", mean_medium_from_matrix(mdata->eps_inv + index), index);
          for (ig = 0; ig < ntot; ++ig)
  ...
  for (iu = 0; iu < ntot; ++iu) {
    ...
    for (i = 0; i < n1; ++i)
      for (j = 0; j < n2; ++j)
        for (k = 0; k < n3; ++k) {
          int index = ((i * n2 + j) * n3 + k);
          double epn = mean_medium_from_matrix(mdata->eps_inv + index);
    ...
    for (ix = 0; ix < nx; ++ix)
      for (iy = 0; iy < ny; ++iy)
        for (iz = 0; iz < nz; ++iz) {
          ...
          val = linear_interpolate(x, y, z, data, dims[0], dims[1], dims[2], 1);
          if (val > 1)
            val = 1;
          else if (val < 0)
            val = 0;
...
#define k2 k
...

        {
          ...
          eps0 = linear_interpolate((i2 + 0.5) / n1, (j2 + 0.5) / n2, (k2 + 0.5) / n3, eps, eps_nx,
                                    eps_ny, eps_nz, 1);
          ...
          if (grad) {
            ...
            p.y = j2 * s2 - c2;
            p.z = k2 * s3 - c3;
            material_grids_addgradient_point(work, p, scalegrad, grids, ngrids);
          }
...
#ifndef SCALAR_COMPLEX
          {
            ...
            if (last_index != 0 && 2 * last_index != last_dim) {
              ...
              eps0 = linear_interpolate((i1c + 0.5) / n1, (i2c + 0.5) / n2, (i3c + 0.5) / n3, eps,
    ...
    mpi_allreduce(work, grad, n, double, MPI_DOUBLE, MPI_SUM, mpb_comm);
  {
    ...
    mpi_allreduce(&valtmp, &val, 1, double, MPI_DOUBLE, MPI_SUM, mpb_comm);
  }
...
void material_grids_match_epsilon_fileB(string filename, number eps_tol) {
  ...
  {
    nlopt_result res;
    res = nlopt_minimize(NLOPT_LD_MMA, n, match_eps_func, &d, lb, ub, u, &func_min, -HUGE_VAL, 0, 0,
                         0, u_tol, 0, 0);
